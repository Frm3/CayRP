...
ESX.RegisterServerCallback('astro_selldrugs:checkDrugs', function(source, cb, LSPD)
...
RegisterServerEvent('astro_selldrugs:sell')
AddEventHandler('astro_selldrugs:sell', function(LSPD)
	local xPlayer = ESX.GetPlayerFromId(source)	
	xDrug = 0
	...
	if drugType ~= nil then
		...
		TriggerClientEvent('astro_selldrugs:stopSell', xPlayer.source, 1, ('~o~'.. xDrug ..' w ilośći '.. drugName..' za ' .. blackMoney .. "$" ))
	end
end)


RegisterServerEvent('astro_selldrugs:triggerDispatch')
AddEventHandler('astro_selldrugs:triggerDispatch', function(pos, street, photo)
	local xPlayers = ESX.GetPlayers()

	local msg = ("^r[^310-07^7] ^6^*Zgłoszenie sprzedaży narkotyków. Ulica: ^7^_"..street)
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		if (xPlayer.job.name == 'police') then
			if photo then
			...
			TriggerClientEvent('astro_selldrugs:showNotify', xPlayer.source, pos, street, msg)
		end
	end
end)
