...
RegisterNetEvent('astro_selldrugs:photoMessage')
AddEventHandler('astro_selldrugs:photoMessage', function(id)
  local myId = PlayerId()
										...
										if GetPedType(_pedHandle) ~= 28 then
											if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(_pedHandle), true) < 0.8 then
												if (ScriptData.lastPed ~= _pedHandle) then
													...
													ScriptData.lspdCount = 1
													if (ScriptData.hasDrugs == true) and (ScriptData.lspdCount >= 1 ) then
														ESX.TriggerServerCallback('astro_selldrugs:checkDrugs', function(cb)
															if cb == 5 then 
...
SellDrugs = function(targetPed, lspdCount)
	...
	TriggerServerEvent('astro_selldrugs:sell', lspdCount)
end
...
FailSellDrugs = function(targetPed)
	...
	ScriptData.lastPed = targetPed
	TriggerEvent('astro_selldrugs:stopSell', 2)
end
...
RegisterNetEvent('astro_selldrugs:stopSell')
AddEventHandler('astro_selldrugs:stopSell', function(soldInt, soldString)
	local playerPed = GetPlayerPed(-1)
	...
	elseif soldInt == 2 then
		...
		if notify == 1 then
			...
			TriggerEvent('astro_selldrugs:photoMessage', GetPlayerServerId(PlayerId()))
			TriggerServerEvent('astro_selldrugs:triggerDispatch', coords, street, true)
			ESX.ShowNotification('~b~Osoba nie jest zainteresowana')
		elseif notify == 2 then
			local coords = GetEntityCoords(playerPed)
			local street = GetStreetNameFromHashKey(GetStreetNameAtCoord(coords.x, coords.y, coords.z))
			TriggerServerEvent('astro_selldrugs:triggerDispatch', coords, street, false)
			ESX.ShowNotification('~b~Osoba nie jest zainteresowana')
...
RegisterNetEvent('astro_selldrugs:showNotify')
AddEventHandler('astro_selldrugs:showNotify', function(pos, street, msg)
