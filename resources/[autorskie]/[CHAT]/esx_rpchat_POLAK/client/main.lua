...
RegisterNetEvent('esx_rpchat:triggerDisplayx')
AddEventHandler('esx_rpchat:triggerDisplayx', function(text, source, color, type)
	local player = GetPlayerFromServerId(source)
    if player ~= -1 then
		local offset = 0 + (nbrDisplaying*0.14)
		Display(GetPlayerFromServerId(source), text, offset, color, type)
	end
end)
...
function Display(mePlayer, text, offset, color, type)
    ...
    CreateThread(function()
        ...
        while displaying do
			...
			--	DrawText3D(coordsMe['x'], coordsMe['y'], coordsMe['z']+ 0.75 +offset, text, color)
					DrawText3Ds2(coordsMe['x'], coordsMe['y'], coordsMe['z']+ 0.75 +offset, text, { 255, 255, 255 }, 0.3, {0.003, 0.02, 325})
				else
					DrawText3Ds(coordsMe['x'], coordsMe['y'], coordsMe['z']+ 0.75 +offset, text, { 255, 255, 255 }, 0.3, {0.003, 0.02, 325})
				end
            end
        end
        nbrDisplaying = nbrDisplaying - 1
...
function DrawText3Ds(x, y, z, text, color, size, rect)
	...
	DrawRect(_x, _y + 0.0125, rect[1] + factor, rect[2], 94, 20, 94, 68)
end

function DrawText3Ds2(x, y, z, text, color, size, rect)
	size = size or 0.4
	rect = rect or {0.005, 0.03, 250}
	...
	local factor = text:len() / rect[3]
	DrawRect(_x, _y + 0.0125, rect[1] + factor, rect[2], 168, 113, 160, 68)
end
