	...
	while true do
		...
		if CurrentBed ~= nil then
			if not OnBed then
				...
				Draw3DText({x = BedCoords.x, y = BedCoords.y, z = (BedCoords.z+1)}, '[~g~E~w~] aby się położyć', 0.35)
			elseif OnBed then
				local BedCoords = GetEntityCoords(CurrentBed)
				Draw3DText({x = BedCoords.x, y = BedCoords.y, z = (BedCoords.z+1)}, '[~g~H~w~] aby wstać', 0.35)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentBed ~= nil and IsControlJustReleased(0, 38) then
			...
			OnBed = true
		elseif IsControlJustReleased(0, 104) and OnBed == true then
