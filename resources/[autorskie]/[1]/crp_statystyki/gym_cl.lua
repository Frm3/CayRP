...
function startExercise(animInfo, pos)
    ...
    Citizen.CreateThread(function()
        while exercising do
            ...
            DrawText2D(0.505, 0.925, 1.0,1.0,0.33, "Procent: " .. color..procent .. "%", 255, 255, 255, 255)
            DrawText2D(0.505, 0.95, 1.0,1.0,0.33, "Naciśnij ~g~[SPACJE]~w~ aby trenować", 255, 255, 255, 255)
            DrawText2D(0.505, 0.975, 1.0,1.0,0.33, "Naciśnij ~r~[DELETE]~w~ aby zatrzymać trening", 255, 255, 255, 255)
        end
    end)
    ...
    Citizen.CreateThread(function()
        while canExercise do
            ...
            if procent <= 99 then
                ...
                ExitTraining(animInfo["exitDict"], animInfo["exitAnim"], animInfo["exitTime"])
                TriggerEvent("FeedM:showNotification", 'Dostałeś +1 do siły!')
		TriggerServerEvent('updatebaza123', 'sila')
            end
        end
    end)
end
