...
RegisterNetEvent('szymczakovv-opis2:send', function(sent)
    scenes = sent
end)
  ...
  ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == "setopis" then
          ...
          function(data2, menu2)
            ...
            if text  == nil then
                ...
                TriggerServerEvent('szymczakovv-opis2:add', coords, message, color, distance)
            end
        ...
        if data.current.value == "deleteopis" then
            ...
            if scene ~= nil then
                TriggerServerEvent('szymczakovv-opis2:delete', scene)
            end
        end
...
TriggerServerEvent('szymczakovv-opis2:fetch')
