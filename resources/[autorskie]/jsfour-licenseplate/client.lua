...
AddEventHandler('jsfour-licenseplate', function()
  ...
  ESX.UI.Menu.Open(
	...
	{
		title    = 'Menu tablicy rejestracyjnej',
    align    = 'bottom-right',
		elements = {
			{label = 'Zmień Rejestracje', value = 'change'},
			{label = 'Zastosuj do samochodu ', value = 'apply'},
		}
	},
	function(data, menu)
      ...
      if plate ~= nil then
        ...
        if distance ~= -1 and distance <= 3.0 then
          ...
          ESX.TriggerServerCallback('jsfour-licenseplate:update', function( cb )
            if cb == 'found' then
              ESX.UI.Menu.CloseAll()
	      TriggerServerEvent('crp_license:changeplate', oldplate, plate)
              SetVehicleNumberPlateText(vehicle, plate)
              plate = nil
            elseif cb == 'error' then
              ESX.ShowNotification('Nie mogłeś zastosować tablicy rejestracyjnej')
            end
          end, oldplate, plate)
        else
        ...
        ESX.ShowNotification('Nie można zastosować pustej rejestracji')
      end
		end
	end,
	function(data, menu)
