...
RegisterServerEvent('crp_license:changeplate')
AddEventHandler('crp_license:changeplate', function(old, new)
        MySQL.Sync.execute("UPDATE veh_km SET carplate = @newplate WHERE carplate = @oldplate", {['@oldplate'] = old, ['@newplate'] = new})
end)
