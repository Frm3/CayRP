...
local banks = {
	...
	{name="Pacific Bank", closed = false, id=267, x=241.727, y=220.706, z=106.286, principal = true},
	...
	while true do
		...
		if nearBank and canUse then 
			if not banks[bankkey].closed then
				if not inMenu then
					sleepThread = 5
					DisplayHelpText("Naciśnij ~INPUT_PICKUP~ dla usług bankowych ~b~")			
				end
			else
				if not inMenu then
					sleepThread = 5
					DisplayHelpText("~r~ BANK JEST ZAMKNIĘTY ~b~")			
				end
			end
