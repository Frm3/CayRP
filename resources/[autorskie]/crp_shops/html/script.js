...
$(function() {
		window.addEventListener('message', function(event) {
			...
			$('#wrapper').append(
				` <h6 class = "h4" style = "right: 47.125%; position: absolute;" bottom = ${id - 5}>Sklep</h6>`
			)
			...
			if (event.data.owner == true) {
				...
				$('#wrapper').append(
					`
					<button class = "button" id = "bossactions" style = "position: absolute; right: 15px; top: 5px;">Akcje Szefa</button>
					`
				);
			}
			...
			$('.image').on('click', function () {
				...
				$.post('http://crp_shops/putcart', JSON.stringify({item: item, price : price, label : label, count : count, id : id}), function( cb ) {
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "Ileee?"></textarea>
							</div>`
								);
							};
							...
							$('#cart').append(
							`
							<button class = "button" id = "buybutton" style = "position: absolute; right: 15px; top: 5px;">Zakup</button>
							<button class = "button" id = "back" style = "position: absolute; left: 15px; top: 5px;">Cofnij</button>
							`
						);
					});
			});
			...
			$("body").on("click", "#refreshcart", function() {
				$.post('http://crp_shops/escape', JSON.stringify({}));
				location.reload(true);
				$('#wrapper').hide("fast");
				$('#payment').hide("fast");
				$('#cart').hide("fast");
				$.post('http://crp_shops/refresh', JSON.stringify({}));
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$('#payment').hide("fast");
						$('#cart').hide("fast");
						$.post('http://crp_shops/buy', JSON.stringify({Count : value[i].value, Item : value[i].id}));
					} 
					else {
						$.post('http://crp_shops/notify', JSON.stringify({msg : "~r~Brakuje zapasów jednego z towarów lub ilość jest nieprawidłowa."}));
					}
				}
			});
			...
			$("body").on("click", "#bossactions", function() {
				$.post('http://crp_shops/bossactions', JSON.stringify({}));
				$.post('http://crp_shops/escape', JSON.stringify({}));
				location.reload(true);
				$.post('http://crp_shops/emptycart', JSON.stringify({}));
				$('#wrapper').hide("fast");
				$('#payment').hide("fast");
				$('#cart').hide("fast");
			});
