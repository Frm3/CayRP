...
function OpenShopCenter()
	...
	ESX.UI.Menu.CloseAll()
  	...
  	local elements = {}
		...
		if showblip then
			table.insert(elements, {label = 'Ukryj WSZYSTKIE sklepy na mapie', value = 'removeblip'})
		else
			table.insert(elements, {label = 'Pokaż WSZYSTKIE sklepy na mapie', value = 'showblip'})
		end
...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'Masz: $' .. data[1].money .. ' na koncie firmowym',    value = ''})
		table.insert(elements, {label = 'Przesyłki ',    value = 'shipments'})
        table.insert(elements, {label = 'Wystaw przedmiot na sprzedaż', value = 'putitem'})
        table.insert(elements, {label = 'Wyjmij przedmiot na sprzedaż',    value = 'takeitem'})
        table.insert(elements, {label = 'Wpłać pieniądze na swój biznes ',    value = 'putmoney'})
        table.insert(elements, {label = 'Wyciągnij pieniądze ze swojej firmy',    value = 'takemoney'})
        table.insert(elements, {label = 'Zmień nazwę w swojej firmie: $' .. Config.ChangeNamePrice,    value = 'changename'})
		table.insert(elements, {label = 'Sprzedaj swoją firmę za $' .. math.floor(data[1].ShopValue / Config.SellValue),   value = 'sell'})
		...
		elseif data.current.value == 'sell' then
		  ...
		  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell', {
			title = 'NAPISZ: (TAK) bez nawiasów do potwierdzenia'
          }, function(data4, menu4)
            
            if data4.value == 'TAK' then
	  ...
	  elseif data.current.value == 'changename' then
		...
		ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'changename', {
		  title = 'Jak chcesz nazwać swój sklep?'
...
function OpenShipments(id)
	...
	table.insert(elements, {label = 'Zamów produkt', value = 'buy'})
	table.insert(elements, {label = 'Przesyłki', value = 'shipments'})
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = '--GOTOWE PRZESYŁKI--'})
			table.insert(elements, {label = 'Odbierz wszystkie swoje przesyłki', value = 'pickup'})
			once2 = false
			end
			...
			if time - items[i].time <= Config.DeliveryTime and once then
				table.insert(elements, {label = '--PRZESYŁKI OCZEKUJĄCE--'})
				once = false
			end

			if time - items[i].time <= Config.DeliveryTime then
				times = time - items[i].time
				table.insert(elements, {label = items[i].label .. ' pozostały czas: ' .. math.floor((Config.DeliveryTime - times) / 60) .. ' minut' })
			end
...
function OpenShipmentDelivery(id)
	...
	local elements = {}
		...
		for i=1, #Config.Items, 1 do
			table.insert(elements, {labels =  Config.Items[i].label, label =  Config.Items[i].label .. ' za $' .. Config.Items[i].price .. ' sztuk ',	value = Config.Items[i].item, price = Config.Items[i].price})
		end
...
function TakeItem(number)
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getShopItems', function(result)
	...
	for i=1, #result, 1 do
	    if result[i].count > 0 then
	    	table.insert(elements, {label = result[i].label .. ' | ' .. result[i].count ..' sztuk w magazynie [' .. result[i].price .. ' $ za sztuke', value = 'removeitem', ItemName = result[i].item})
	    end
    end
    ...
    for i=1, #result.items, 1 do
      ...
      local invitem = result.items[i]
	    ...
	    if invitem.count > 0 then
			table.insert(elements, { label = invitem.label .. ' | ' .. invitem.count .. ' w twojej torbie', count = invitem.count, name = invitem.name})
	    end
	end
  ...
  function(data, menu)
        ...
        local invcount = data.current.count
			...
			if count > invcount then
				ESX.ShowNotification('~r~Nie możesz sprzedać więcej niż posiadasz!')
				menu2.close()
				menu.close()
			else
				menu2.close()
...
Citizen.CreateThread(function()
	Citizen.Wait(500)
		...
		AddTextComponentString('Kup Własny Sklep')
