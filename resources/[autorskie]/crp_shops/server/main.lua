      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Masz już ten sam przedmiot w swoim sklepie, ~r~ale za ' .. data[1].price .. '. ustalasz cenę ' .. Price)
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Usuń przedmiot i podaj nową cenę lub ustaw tę samą cenę')
            end
        end)  
    end)
end)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent('esx:showNotification', src, '~r~Nie masz wystarczająco dużo pieniędzy.')
            elseif ItemCount <= 0 then
                TriggerClientEvent('esx:showNotification', src, '~r~nieprawidłowa ilość.')
            else
                xPlayer.removeMoney(ItemCount * result[1].price)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Kupiłeś/aś ' .. ItemCount .. 'x ' .. Item .. ' za $' .. ItemCount * result[1].price)
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~Zamówiłeś ' .. count .. ' sztuk ' .. label .. ' za $' .. price * count)
        else
            TriggerClientEvent('esx:showNotification', _source, '~r~Nie masz wystarczająco dużo pieniędzy w swoim sklepie.')
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~gKupiłeś sklep za $' ..  price)
        else    
            TriggerClientEvent('esx:showNotification', _source, '~r~Nie stać cię na ten sklep')
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Twoje pieniądze w sklepie zostały zablokowane z powodu napadu, proszę czekać ' .. math.floor((900 - time) / 60) .. ' minut')
            return
        end
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Wprowadzasz do swojego sklepu '..amount..'$')
        else
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Nie możesz włożyć więcej niż posiadasz')
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Pobrałeś $'..amount..' ze swojego sklepu')
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Sprzedałeś swój sklep')
            else
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Nie możesz sprzedawać swojego sklepu z przedmiotami lub pieniędzmi w środku')
            end
        end)
    end)
end)
