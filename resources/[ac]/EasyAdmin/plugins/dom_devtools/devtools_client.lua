...
AddEventHandler("EasyAdmin:BuildMainMenuOptions", function() 
	...
	if permissions["devtools_coords"] then
	...
	if permissions["devtools_noclip"] then
	...
	if permissions["devtools_vanish"] then
		local thisItem = NativeUI.CreateCheckboxItem('Niewidzialnosc', not IsEntityVisible(PlayerPedId()), "")
		trainerMenu:AddItem(thisItem)
		thisItem.CheckboxEvent = function(sender, item, status)
			if item == thisItem then
				local playerPed = PlayerPedId()
	...
	if permissions["devtools_tpm"] then
