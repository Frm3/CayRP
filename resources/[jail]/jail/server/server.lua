	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent("esx:showNotification", src, GetPlayerName(jailPlayer) .. " Skazany na " .. jailTime .. " minut!")
				
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "SĘDZIA",  Firstname .. " " .. Lastname .. " Jest teraz w więzieniu z powodu: " .. args[3] .. "Na: " .. jailTime .. " Miesięcy" }, color = { 249, 166, 0 } })
					end)
				end
			else
				TriggerClientEvent("esx:showNotification", src, "Ten czas jest niewłaściwy!")
			end
		else
			TriggerClientEvent("esx:showNotification", src, "Ten identyfikator nie jest online!")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Nie jesteś oficerem!")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "SĘDZIA",  Firstname .. " " .. Lastname .. " Jest teraz w więzieniu z powodu: " .. jailReason .. " Na: " .. jailTime .. " Miesięcy" }, color = { 249, 166, 0 } })
	end)

	TriggerClientEvent("esx:showNotification", src, GetPlayerName(targetSrc) .. " Skazany na " .. jailTime .. " minut!")
end)
...
AddEventHandler("esx-qalle-jail:unJailPlayer", function(targetIdentifier)
	...
	TriggerClientEvent("esx:showNotification", src, xPlayer.name .. " Uwolniony!")
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Dzięki, masz trochę gotówki na jedzenie!")
end)
