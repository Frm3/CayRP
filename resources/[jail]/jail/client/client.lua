...
function JailLogin()
	...
	ESX.ShowNotification("Kiedy ostatnio kładłeś się spać, zostałeś uwięziony, z tego powodu jesteś teraz umieszczony z powrotem!")
...
function UnJail()
	...
	ESX.ShowNotification("Jesteś zwolniony, zachowaj spokój na zewnątrz! Powodzenia!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Zostało ci " .. jailTime .. " minut w więzieniu!")
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Już wziąłeś tą paczke")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Otwarte drzwi", 0.4)
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Zostaw paczke", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Najbliższa osoba do więzieniu", value = "jail_closest_player" },
				{ label = "Wypuść osobe z więzienia", value = "unjail_player" }
			}
		}, 
	function(data, menu)
            	...
            	if jailTime == nil then
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Musisz coś tu umieścić!")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Twoje więzienie jest puste!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Więzień: " .. playerArray[i].name .. " | Czas więzienia: " .. playerArray[i].jailTime .. " minutes", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Wypuść gracza z więzienia",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
