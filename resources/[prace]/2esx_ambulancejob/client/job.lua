...
print(zone)
	...
	elseif zone == 'AmbulanceActions' then
		...
		CurrentActionMsg	= _U('open_menu')
		CurrentActionData	= {}
	elseif zone == 'AmbulanceActions1' then
		CurrentAction		= 'ambulance_actions_menu'
		CurrentActionMsg	= _U('open_menu')
		CurrentActionData	= {}
	elseif zone == 'VehicleSpawner' then
		...
		CurrentActionData	= {}
	elseif zone == 'VehicleSpawner1' then
...
local blips = {
	{title="Szpital", colour=2, id=61, x=297.1, y=-584.21, z=43.13}
}
	...
	ESX.UI.Menu.CloseAll()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner2',
		{
			...
			menu.close()
            	...
            	if data.current.model == 'uh1nasa' then
			ESX.Game.SpawnVehicle(data.current.model, Config.Zones.HelkaSpawnPoint2.Pos, 337.41, function(vehicle)
				local playerPed = PlayerPedId()
				TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
			end)
		else
			ESX.Game.SpawnVehicle(data.current.model, Config.Zones.VehicleSpawnPoint1.Pos, 337.2, function(vehicle)
				local playerPed = PlayerPedId()
				TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
			end)
		end
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
		{
			...
			menu.close()
            	...
            	if data.current.model == 'uh1nasa' then
			ESX.Game.SpawnVehicle(data.current.model, Config.Zones.HelkaSpawnPoint.Pos, 337.41, function(vehicle)
				local playerPed = PlayerPedId()
				TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
			end)
		else
			ESX.Game.SpawnVehicle(data.current.model, Config.Zones.VehicleSpawnPoint.Pos, 337.2, function(vehicle)
				local playerPed = PlayerPedId()
				TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
			end)
		end
