...
AddEventHandler('esx_sadownik:startHarvest', function(zone)
	...
	if itemQuantity == 20 or itemQuantity > 20 then
		TriggerClientEvent("FeedM:showNotification", "Nie masz juz miejsca w torbie na jablka")
	return
	end
...
AddEventHandler('esx_sadownik:startHarvest2', function(zone)
	...
	if itemQuantity == 20 or itemQuantity > 20 then
		TriggerClientEvent("FeedM:showNotification", "Nie masz juz miejsca w torbie na pomarancze")
	return
end
...
AddEventHandler('esx_sadownik:startTransform', function(zone)
	...
	if jablka < 5 then
		TriggerClientEvent("FeedM:showNotification", 'Nie posiadasz wystarczajacej ilosci jablek')
	elseif pomarancza < 5 then
		TriggerClientEvent("FeedM:showNotification", 'Nie posiadasz wystarczajacej ilosci pomaranczy')
	elseif sok2 == 40 or sok2 > 40 then
		TriggerClientEvent("FeedM:showNotification", "Posiadasz za duzo soku przy sobie")
	return
	else
			...
			if sok == 0 then
				...
				if (sok == 1) then
					SetTimeout(20000, function()
						xPlayer.removeInventoryItem('sok', 40)
						xPlayer.addMoney('30') --pieni�dze za jaki sprzedajemy sok x5
						Sell(source,zone)
...
AddEventHandler('esx_sadownik:startSell', function(zone)
	...
	if itemQuantity < 5  then
		TriggerClientEvent("FeedM:showNotification", "Nie posiadasz soku do sprzedania")
	return
	end
...
AddEventHandler('esx_sadownik:startSell3', function(zone)
	...
	if itemQuantity < 5  then
		TriggerClientEvent("FeedM:showNotification", "Tym razem mnie nie okradniesz!")
	else
